(function () {
    ...
    function initUtilityBar() {
        var timeEl = qs("#current-time");
        var uptimeEl = qs("#system-uptime");

        function updateTime() {
            var now = new Date();
            var timeStr = now.toLocaleTimeString('en-US', { hour12: false, hour: '2-digit', minute: '2-digit', second: '2-digit' });
            if (timeEl) setText(timeEl, timeStr + " EST");

            // Calculate and display simulated uptime
            var diff = Math.floor((Date.now() - startTime) / 1000);
            var hours = Math.floor(diff / 3600);
            var minutes = Math.floor((diff % 3600) / 60);
            var seconds = diff % 60;
            var uptimeStr = [hours, minutes, seconds].map(v => v.toString().padStart(2, '0')).join(':');
            if (uptimeEl) setText(uptimeEl, "Uptime: " + uptimeStr);
            
            if (w.requestAnimationFrame) w.requestAnimationFrame(updateTime);
            else w.setTimeout(updateTime, 1000); // Fallback
        }
        
        if (!prefersReducedMotion) updateTime();
        else {
             if (timeEl) setText(timeEl, "10:45:00 EST");
             if (uptimeEl) setText(uptimeEl, "Uptime: 00:47:00");
        }
    }
    ...
    function renderRespond() {
        ...
        var statusBadge = qs("#response-status-badge");
        
        // NOTE: The HTML already contains the static structure for the custom-response-dashboard.
        // We only populate the timeline list and key points from JSON data.
        ...
        // Reset status badge display for initial load (before scrollama hits containment step)
        if (statusBadge) {
             statusBadge.textContent = "RESPONSE ACTIVE";
             statusBadge.classList.remove('status-contained');
    ...
    function renderForensics() {
        ...
        // Populate Sticky Panel Evidence Grid (Panel data)
        setHTML(grid, "");
        ...
        // Populate Flow Narrative Points (Story text)
        setHTML(points, "");
        ...
        // NOTE: Dynamic population of forensics-timeline, forensics-grid, etc. is omitted
        // as the HTML contains the static structure for these, and they are not defined in the JSON.
    }

    function renderLessons() {
        var heatmap = qs("#control-heatmap");
        var points = qs("#lessons-points");
        var metricGrid = qs(".lessons-metric-grid");
        var remediationTableBody = qs(".remediation-table tbody");
        
        if (!heatmap || !points) return;

        // 1. Populate Sticky Panel Heatmap
        setHTML(heatmap, "");
        ...
        // 2. Populate Flow Narrative Points
        setHTML(points, "");
        ...
        // 3. Populate Metrics Grid (assuming LESSONS.metrics is present in JSON)
        if (metricGrid && safeArray(LESSONS.metrics).length) {
            setHTML(metricGrid, "");
            safeArray(LESSONS.metrics).forEach(function(m) {
                var gauge = create("div", "metric-gauge");
                gauge.setAttribute("data-impact", m.impact || "low");
                
                var value = create("div", "gauge-value"); setText(value, m.value);
                var label = create("div", "gauge-label"); setText(label, m.label);

                gauge.appendChild(value);
                gauge.appendChild(label);
                metricGrid.appendChild(gauge);
            });
        }
        
        // 4. Populate Remediation Table (assuming LESSONS.remediation is present in JSON)
        if (remediationTableBody && safeArray(LESSONS.remediation).length) {
            setHTML(remediationTableBody, "");
            safeArray(LESSONS.remediation).forEach(function(row) {
                 var tr = create("tr");
                 
                 var statusClass = (row.status || "").toLowerCase().replace(' ', '-');

                 tr.innerHTML = `
                    <td data-label="Finding">${row.finding || ''}</td>
                    <td data-label="Action">${row.action || ''}</td>
                    <td data-label="Status" class="status-${statusClass}">${row.status || ''}</td>
                    <td data-label="Risk Reduction">${row.risk || ''}</td>
                 `;
    ...
    function setupScrollama() {
        ...
        stageKeys.forEach(function (id) {
            ensureScroller(id).setup({
                ...
                var statusBadge = qs("#response-status-badge");
                var containmentStep = qs('[data-containment-step="true"]');
                ...
                // Response Status Update logic
                if (id === "respond" && statusBadge && containmentStep) {
                    var containmentStepIndex = qsa('.timeline-visual .timeline-step').indexOf(containmentStep);
                    var currentStoryIndex = response.index;

                    // Assuming containment is achieved when the user scrolls past the first story step (index 0)
                    if (currentStoryIndex >= 0) {
                         // Check if the actual containment step is visually passed (e.g., assuming it's the 3rd element, index 2 in the timeline-visual)
                         // The story itself is one long block, so we trigger containment once the story is active.
                         
                         statusBadge.textContent = "CONTAINED";
                         statusBadge.classList.remove('status-active');
                         statusBadge.classList.add('status-contained');
                    }
                }
            })
            .onStepExit(function (response) {
                 // Reset containment status when scrolling up out of the respond section
                 if (response.direction === 'up' && response.index === 0 && id === 'respond') {
                     var statusBadge = qs("#response-status-badge");
                     if (statusBadge) {
                         statusBadge.textContent = "RESPONSE ACTIVE";
                         statusBadge.classList.remove('status-contained');
    ...
    function fetchTechReviewRSS() {
        ...
        fetch(PROXY_URL)
            ...
            .then(function (payload) {
                ...
                var localPosts = safeArray(DATA.blog_posts);

                if (items.length) {
                    var rssPosts = items.slice(0, 3).map(function (item) { // Limit RSS posts
                        return {
                            title: "[AI/THREAT] " + item.title || "", // Highlight as external/AI related
                            summary: truncate(stripHtml(item.description), 260),
                            tags: safeArray(item.categories).slice(0, 2),
                            link: item.link || ""
                        };
                    });

                    BLOG_POSTS = rssPosts.concat(localPosts); // Prepend RSS posts
                } else {
                    BLOG_POSTS = localPosts; // Fallback to local if fetch succeeds but returns no items
                }

                renderBlog();
            })
            .catch(function () {
                // If fetching RSS fails (error or timeout), ensure local posts are still rendered
                BLOG_POSTS = safeArray(DATA.blog_posts);
                renderBlog();
            });
    }
